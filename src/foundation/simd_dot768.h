...
static inline float fce_dot768_avx2(const float * restrict a,
...
__attribute__((target("avx2,fma")))
static inline void fce_dot768_mags3_avx2(const float * restrict a,
                                           const float * restrict b,
    ...
    for (int i = 0; i < FCE_SIMD_DIM; i += 32) {
        ...
        d0  = _mm256_fmadd_ps(ai0, bi0, d0);  d1  = _mm256_fmadd_ps(ai1, bi1, d1);
        d2  = _mm256_fmadd_ps(ai2, bi2, d2);  d3  = _mm256_fmadd_ps(ai3, bi3, d3);
        ma0 = _mm256_fmadd_ps(ai0, ai0, ma0); ma1 = _mm256_fmadd_ps(ai1, ai1, ma1);
        ma2 = _mm256_fmadd_ps(ai2, ai2, ma2); ma3 = _mm256_fmadd_ps(ai3, ai3, ma3);
        mb0 = _mm256_fmadd_ps(bi0, bi0, mb0); mb1 = _mm256_fmadd_ps(bi1, bi1, mb1);
        mb2 = _mm256_fmadd_ps(bi2, bi2, mb2); mb3 = _mm256_fmadd_ps(bi3, bi3, mb3);
    }
    ...
    ds  = _mm_hadd_ps(ds, ds);  ds  = _mm_hadd_ps(ds, ds);
    as_ = _mm_hadd_ps(as_, as_); as_ = _mm_hadd_ps(as_, as_);
    bs  = _mm_hadd_ps(bs, bs);  bs  = _mm_hadd_ps(bs, bs);
...
__attribute__((target("avx2,fma")))
static inline void fce_dot768_add_mag_b_avx2(const float * restrict a,
                                               const float * restrict b,
    ...
    for (int i = 0; i < FCE_SIMD_DIM; i += 32) {
        ...
        d0 = _mm256_fmadd_ps(ai0, bi0, d0); d1 = _mm256_fmadd_ps(ai1, bi1, d1);
        d2 = _mm256_fmadd_ps(ai2, bi2, d2); d3 = _mm256_fmadd_ps(ai3, bi3, d3);
        m0 = _mm256_fmadd_ps(bi0, bi0, m0); m1 = _mm256_fmadd_ps(bi1, bi1, m1);
        m2 = _mm256_fmadd_ps(bi2, bi2, m2); m3 = _mm256_fmadd_ps(bi3, bi3, m3);
    }
    d0 = _mm256_add_ps(_mm256_add_ps(d0,d1), _mm256_add_ps(d2,d3));
    ...
    ds = _mm_hadd_ps(ds, ds); ds = _mm_hadd_ps(ds, ds);
    ms = _mm_hadd_ps(ms, ms); ms = _mm_hadd_ps(ms, ms);
    *out_dot = _mm_cvtss_f32(ds);
...
static inline void fce_dot768_mags3_neon(const float * restrict a,
    ...
    for (int i = 0; i < FCE_SIMD_DIM; i += 16) {
        ...
        d0 = vmlaq_f32(d0, ai0, bi0); d1 = vmlaq_f32(d1, ai1, bi1);
        d2 = vmlaq_f32(d2, ai2, bi2); d3 = vmlaq_f32(d3, ai3, bi3);
        a0 = vmlaq_f32(a0, ai0, ai0); a1 = vmlaq_f32(a1, ai1, ai1);
        a2 = vmlaq_f32(a2, ai2, ai2); a3 = vmlaq_f32(a3, ai3, ai3);
        b0 = vmlaq_f32(b0, bi0, bi0); b1 = vmlaq_f32(b1, bi1, bi1);
        b2 = vmlaq_f32(b2, bi2, bi2); b3 = vmlaq_f32(b3, bi3, bi3);
    }
...
static inline void fce_dot768_add_mag_b_neon(const float * restrict a,
    ...
    for (int i = 0; i < FCE_SIMD_DIM; i += 16) {
        ...
        d0 = vmlaq_f32(d0, ai0, bi0); d1 = vmlaq_f32(d1, ai1, bi1);
        d2 = vmlaq_f32(d2, ai2, bi2); d3 = vmlaq_f32(d3, ai3, bi3);
        m0 = vmlaq_f32(m0, bi0, bi0); m1 = vmlaq_f32(m1, bi1, bi1);
        m2 = vmlaq_f32(m2, bi2, bi2); m3 = vmlaq_f32(m3, bi3, bi3);
    }
    d0 = vaddq_f32(vaddq_f32(d0,d1), vaddq_f32(d2,d3));
...
static inline void fce_dot768_mags3_scalar(const float * restrict a,
    ...
    *out_dot = dot; *out_ma = ma; *out_mb = mb;
}
...
static inline void fce_dot768_mags3(const float * restrict a,
                                     ...
                                     float *out_mb) {
#if FCE_HAS_AVX2
    if (fce_has_avx2()) { fce_dot768_mags3_avx2(a, b, out_dot, out_ma, out_mb); return; }
#elif FCE_HAS_NEON
    fce_dot768_mags3_neon(a, b, out_dot, out_ma, out_mb); return;
#endif
...
static inline void fce_dot768_add_mag_b(const float * restrict a,
                                         ...
                                         float *out_dot, float *out_mb) {
#if FCE_HAS_AVX2
    if (fce_has_avx2()) { fce_dot768_add_mag_b_avx2(a, b, out_dot, out_mb); return; }
#elif FCE_HAS_NEON
    fce_dot768_add_mag_b_neon(a, b, out_dot, out_mb); return;
#endif
...
#if FCE_HAS_AVX2
    if (fce_has_avx2()) { fce_axpy_f32_768_avx2(dst, src, scale); return; }
#elif FCE_HAS_NEON
    fce_axpy_f32_768_neon(dst, src, scale); return;
#endif
...
#if FCE_HAS_AVX2
    if (fce_has_avx2()) { fce_axpy_i8_768_avx2(dst, src, scale); return; }
#elif FCE_HAS_NEON
    fce_axpy_i8_768_neon(dst, src, scale); return;
#endif
...
#if FCE_HAS_AVX2
    if (fce_has_avx2()) { fce_init_f32_from_i8_768_avx2(dst, src, scale); return; }
#elif FCE_HAS_NEON
    fce_init_f32_from_i8_768_neon(dst, src, scale); return;
#endif
...
static inline void fce_quantize_f32_768(int8_t * restrict out,
                                         const float * restrict src) {
#if FCE_HAS_AVX2
    if (fce_has_avx2()) { fce_quantize_f32_768_avx2(out, src); return; }
#elif FCE_HAS_NEON
    fce_quantize_f32_768_neon(out, src); return;
#endif
