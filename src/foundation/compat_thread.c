...
#define FCE_DEFAULT_STACK_SIZE ((size_t)1 * FCE_SZ_1K * FCE_SZ_1K)
#include <string.h>
...
static BOOL WINAPI win_once_callback(PINIT_ONCE InitOnce, PVOID Parameter, PVOID *Context) {
    (void)InitOnce;
    (void)Context;
    win_once_arg_t *arg = (win_once_arg_t *)Parameter;
    arg->fn();
    return TRUE;
}

int fce_once(fce_once_t *once, void (*fn)(void)) {
    win_once_arg_t arg = {fn};
    if (!InitOnceExecuteOnce(once, win_once_callback, &arg, NULL)) return -1;
    return 0;
}
